...
func testRepliesFind(t *testing.T) {
	...
	replyFound, err := FindReply(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	replyDBTypes = map[string]string{`ID`: `character varying`, `CreatedAt`: `timestamp without time zone`, `AuthorID`: `character varying`, `Content`: `text`, `CommentID`: `character varying`}
	_            = bytes.MinRead
)
